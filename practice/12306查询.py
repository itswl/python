...
VerificationCodeurl="https://kyfw.12306.cn/passport/captcha/captcha-image?login_site=E&module=login&rand=sjrand"
while True:
    urllib.request.urlretrieve(VerificationCodeurl,"./12306_VerificationCode.png")
    VerificationCode=input("请输入验证码，输入第几张图片即可")
    if(VerificationCode!="re"):
        break
#x坐标(35,112,173,253)，y坐标(45)
#x坐标(35,112,173,253)，y坐标(114)
pat1='"(.*?)"'
allpic=re.compile(pat1).findall(VerificationCode)
def getxy(pic):
    if(pic==1):
        xy=(35,45)
    if(pic==2):
...
print(allpicpos2)
#post验证码验证
VerificationCodeposturl="https://kyfw.12306.cn/passport/captcha/captcha-check"
VerificationCodepostdata =urllib.parse.urlencode({
"answer":allpicpos2,
"rand":"sjrand",
"login_site":"E",
}).encode('utf-8')
req1 = urllib.request.Request(VerificationCodeposturl,VerificationCodepostdata)
